...
public abstract class KeyPair implements java.io.Serializable {
    ...
    private static final byte[] cr = proxy.Tools.str2byte("\n");

    public static KeyPair genKeyPair(Configure jsch, int type) throws ProxyException {
        return genKeyPair(jsch, type, 1024);
    }

    public static KeyPair genKeyPair(Configure jsch, int type, int key_size) throws ProxyException {
        KeyPair kpair = null;
        if (type == DSA) {
    ...
    abstract void generate(int key_size) throws ProxyException;
    ...
    public abstract byte[] forSSHAgent() throws ProxyException;
    ...
    public KeyPair(Configure jsch) {
        this.jsch = jsch;
    }

    static byte[][] header = {proxy.Tools.str2byte("Proc-Type: 4,ENCRYPTED"),
        proxy.Tools.str2byte("DEK-Info: DES-EDE3-CBC,")};
    ...
    public void writePrivateKey(java.io.OutputStream out, byte[] passphrase) {
        ...
        if (encoded != plain) {
            proxy.Tools.bzero(plain);
        }
        byte[] iv = _iv[0];
        byte[] prv = proxy.Tools.toBase64(encoded, 0, encoded.length);
    ...
    private static byte[] space = proxy.Tools.str2byte(" ");
    ...
    public byte[] getPublicKeyBlob() {
        // TODO ProxyException should be thrown
        //if(publickeyblob == null)
        //  throw new ProxyException("public-key blob is not available");
    ...
    public void writePublicKey(java.io.OutputStream out, String comment) {
        ...
        byte[] pub = proxy.Tools.toBase64(pubblob, 0, pubblob.length);
        try {
    ...
    public void writeSECSHPublicKey(java.io.OutputStream out, String comment) {
        ...
        byte[] pub = proxy.Tools.toBase64(pubblob, 0, pubblob.length);
        try {
            out.write(proxy.Tools.str2byte("---- BEGIN SSH2 PUBLIC KEY ----"));
            out.write(cr);
            out.write(proxy.Tools.str2byte("Comment: \"" + comment + "\""));
            out.write(cr);
            int index = 0;
            ...
            out.write(proxy.Tools.str2byte("---- END SSH2 PUBLIC KEY ----"));
            out.write(cr);
        } catch (Exception e) {
        }
    }
    ...
    public String getFingerPrint() {
        ...
        return proxy.Tools.getFingerPrint(hash, kblob);
    }
    ...
    public void setPassphrase(String passphrase) {
        if (passphrase == null || passphrase.length() == 0) {
            ...
            setPassphrase(proxy.Tools.str2byte(passphrase));
        }
    }
    ...
    public boolean decrypt(String _passphrase) {
        ...
        return decrypt(proxy.Tools.str2byte(_passphrase));
    }
    ...
    public static KeyPair load(Configure jsch, String prvkey) throws ProxyException {
    ...
    public static KeyPair load(Configure jsch, String prvfile, String pubfile) throws ProxyException {

        byte[] prvkey = null;
        byte[] pubkey = null;

        try {
            prvkey = proxy.Tools.fromFile(prvfile);
        } catch (IOException e) {
        ...
        try {
            pubkey = proxy.Tools.fromFile(_pubfile);
        } catch (IOException e) {
            if (pubfile != null) {
                throw new ProxyException(e.toString(), (Throwable) e);
            }
        ...
        try {
            ...
            proxy.Tools.bzero(prvkey);
        }
    }

    public static KeyPair load(Configure jsch, byte[] prvkey, byte[] pubkey) throws ProxyException {
        ...
        if (pubkey == null
            ...
            if (_type.equals("ssh-rsa")) {
                ...
                throw new ProxyException("privatekey: invalid key " + new String(prvkey, 4, 7));
            }
            return kpair;
        }
        ...
        try {
            ...
            while (i < len) {
                if (buf[i] == 'B' && i + 3 < len && buf[i + 1] == 'E' && buf[i + 2] == 'G' && buf[i + 3] == 'I') {
                    i += 6;
                    if (i + 2 >= len) {
                        throw new ProxyException("invalid privatekey: " + prvkey);
                    }
                ...
                if (buf[i] == 'A' && i + 7 < len && buf[i + 1] == 'E' && buf[i + 2] == 'S' && buf[i + 3] == '-'
                    ...
                    if (SessionSSH.checkCipher((String) jsch.getConfig("aes256-cbc"))) {
                        ...
                        throw new ProxyException("privatekey: aes256-cbc is not available " + prvkey);
                    }
                    continue;
                }
                if (buf[i] == 'A' && i + 7 < len && buf[i + 1] == 'E' && buf[i + 2] == 'S' && buf[i + 3] == '-'
                    ...
                    if (SessionSSH.checkCipher((String) jsch.getConfig("aes192-cbc"))) {
                        ...
                        throw new ProxyException("privatekey: aes192-cbc is not available " + prvkey);
                    }
                    continue;
                }
                if (buf[i] == 'A' && i + 7 < len && buf[i + 1] == 'E' && buf[i + 2] == 'S' && buf[i + 3] == '-'
                    ...
                    if (SessionSSH.checkCipher((String) jsch.getConfig("aes128-cbc"))) {
                        ...
                        throw new ProxyException("privatekey: aes128-cbc is not available " + prvkey);
                    }
                    continue;
                }
            ...
            if (buf != null) {
                ...
                if (i - start > 0) {
                    data = proxy.Tools.fromBase64(_buf, start, i - start);
                }

                proxy.Tools.bzero(_buf);
            }
            ...
            if (data != null
                ...
                String _cipher = proxy.Tools.byte2str(_buf.getString());
                ...
                if (_cipher.equals("3des-cbc")) {
                    ...
                    data = foo;
                    encrypted = true;
                    throw new ProxyException("unknown privatekey format: " + prvkey);
            ...
            if (pubkey != null) {
                try {
                    ...
                    if (buf.length > 4 && // FSecure's public key
                        ...
                        if (valid) {
                            publickeyblob = proxy.Tools.fromBase64(buf, start, i - start);
    ...
    static KeyPair loadPPK(Configure jsch, byte[] buf) throws ProxyException {
        byte[] pubkey = null;
        byte[] prvkey = null;
        int lines = 0;
        ...
        prvkey = proxy.Tools.fromBase64(prvkey, 0, prvkey.length);
        pubkey = proxy.Tools.fromBase64(pubkey, 0, pubkey.length);
        ...
        if (kpair.encrypted) {
            if (SessionSSH.checkCipher((String) jsch.getConfig("aes256-cbc"))) {
                try {
                    ...
                    throw new ProxyException("The cipher 'aes256-cbc' is required, but it is not available.");
                }
            } else {
                throw new ProxyException("The cipher 'aes256-cbc' is required, but it is not available.");
            }
