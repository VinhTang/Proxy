...
public class KnownHosts implements HostKeyRepository {
    ...
    private void addInvalidLine(String line) throws ProxyException {
        ...
        synchronized (pool) {
            for (int i = 0; i < pool.size(); i++) {
                ...
                if (hk.isMatched(host) && hk.type == type) {
                    /*
                     if(proxy.Tools.array_equals(hk.key, key)){ return; }
        ...
        synchronized (pool) {
            for (int i = 0; i < pool.size(); i++) {
                ...
                if (host == null
                        || (hk.isMatched(host)
                        && (type == null || (hk.getType().equals(type)
                        && (key == null || proxy.Tools.array_equals(key, hk.key)))))) {
    ...
    HostKey createHashedHostKey(String host, byte[] key) throws ProxyException {
    ...
    class HashedHostKey extends HostKey {
        ...
        HashedHostKey(String host, byte[] key) throws ProxyException {
            this(host, GUESS, key);
        }
        ...
        HashedHostKey(String marker, String host, int type, byte[] key, String comment) throws ProxyException {
            ...
            if (this.host.startsWith(HASH_MAGIC)
                ...
                salt = proxy.Tools.fromBase64(proxy.Tools.str2byte(_salt), 0, _salt.length());
                hash = proxy.Tools.fromBase64(proxy.Tools.str2byte(_hash), 0, _hash.length());
        ...
        void hash() {
            ...
            try {
                synchronized (macsha1) {
                    macsha1.init(salt);
                    byte[] foo = proxy.Tools.str2byte(host);
                    macsha1.update(foo, 0, foo.length);
            ...
            host = HASH_MAGIC + proxy.Tools.byte2str(proxy.Tools.toBase64(salt, 0, salt.length))
                    + HASH_DELIM + proxy.Tools.byte2str(proxy.Tools.toBase64(hash, 0, hash.length));
            hashed = true;
        }
    }
}
